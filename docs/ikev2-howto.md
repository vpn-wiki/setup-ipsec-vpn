...
Alternatively, you may connect using the command line. See [#1399](https://github.com/vpn-wiki/setup-ipsec-vpn/issues/1399), [#1007](https://github.com/vpn-wiki/setup-ipsec-vpn/issues/1007) and [#1789](https://github.com/vpn-wiki/setup-ipsec-vpn/issues/1789) for example steps. If you encounter error `Could not find source connection`, edit `/etc/netplan/01-netcfg.yaml` and replace `renderer: networkd` with `renderer: NetworkManager`, then run `sudo netplan apply`. If using `nmcli` to connect to the VPN, run `sudo nmcli c up VPN`. To disconnect: `sudo nmcli c down VPN`.
